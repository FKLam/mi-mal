<template>
  <div class="order-confirm">
        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" :class="{'checked': checkIndex === index}" v-for="(item, index) in list" :key="index" @click="checkIndex = index">
    ...
    <modal
      :title="userAction === 2 ? '确认删除' : userAction === 0 ? '新增确认' : '编辑地址'"
      :btnType="userAction === 2 ? '1' : '3'"
      :showModal="showEditModal"
      @cancel="showEditModal=false"
      @submit="submitAddress"
    >
      <template v-slot:body>
        <div v-if="userAction === 2">
          <span>确认要此收货地址删除吗？</span>
        </div>
        <div class="edit-wrap" v-else-if="userAction === 0 || userAction === 1">
          <div class="item">
...
export default {
  ...
  methods: {
    ...
    orderSubmit () {
      if (this.list.length <= this.checkIndex) {
        this.$message.warning('请选择一个收货地址')
        return
      }
      let shipping = this.list[this.checkIndex]
